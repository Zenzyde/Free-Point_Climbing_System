...
public class PlayerClimbingNew : PlayerStateInterface
{
	private Vector3 m_moveSpeed;
	private Vector3 move;
	private Transform transform;
	private Camera m_cam;
	private float m_maxLerpCheck, m_maxClimbEnterAngle, m_normalOffset, m_maxClimbAroundOuterAngle, m_maxClimbAroundInnerAngle;
	private Rigidbody m_rigid;
	private Transform m_jumpVisual;
	private CinemachineFreeLook m_freeCam;
	...
	public PlayerClimbingNew(PlayerMovementController moveBase, Transform transform, Vector3 moveSpeed, Camera cam, float lerpCheck, Rigidbody rigid, Transform jumpVisual,
	float climbEnterAngle, float normalOffset, float climbAroundOuterAngle, float climbAroundInnerAngle, CinemachineFreeLook freeCam)
	{
		...
		this.m_maxClimbAroundOuterAngle = climbAroundOuterAngle;
		this.m_maxClimbAroundInnerAngle = climbAroundInnerAngle;
		this.m_freeCam = freeCam;
	}
	...
	void Movement()
	{
		...
		//Check for available wall surface, either to the side or right in front of the player
		if (!CheckForWallRays())
		{
			//No available wall surface, check if player is moving upwards & attempt flooring
			...
			//No available wall surface, not going upwards or no flooring found, check for wall around the corner & attempt climb around
			bool attemptStatus = TryGoAroundWall();
			if (!attemptStatus)
				return;
		}
		else //Available wall surface to the side or right in front of player, align & move
		{
			AlignToWall();
	...
	bool CheckForWallRays()
	{
		...
		Ray rayForward = new Ray(transform.position + move.normalized * 0.5f, transform.forward);
		if (Physics.Raycast(rayDirection.origin, rayDirection.direction, out RaycastHit hit, 1.2f))
		{
			if (hit.transform.CompareTag("Climb"))
			{
				if (VerifyClimbAroundInnerWallAngle(hit.normal))
				{
					return true;
				}
				else if (!VerifyClimbAroundInnerWallAngle(hit.normal))
				{
					return false;
				}
			}
		}
		if (Physics.Raycast(rayForward.origin, rayForward.direction, out hit, .75f))
		{
			if (hit.transform.CompareTag("Climb"))
			{
				return true;
	...
	bool CheckForFloor()
	{
		...
		if (Physics.Raycast(rayTopDown.origin, rayTopDown.direction, out RaycastHit hit, 1.2f))
		{
	...
	void AlignToWall()
	{
		...
		if (Physics.Raycast(rayDirection, out RaycastHit hit, 1.1f))
		{
			if (hit.transform.CompareTag("Climb"))
			{
	...
	bool TryGoAroundWall()
	{
		...
		Ray rayForward = new Ray(transform.position + move.normalized, transform.forward);
		if (Physics.Raycast(rayForward.origin, rayForward.direction, .75f))
		{
			return false;
		}
		if (Physics.Raycast(rayAngled, out RaycastHit hit, 1f))
		{
			//If normal compared to player forward isn't too steep
			if (VerifyClimbAroundOuterWallAngle(hit.normal))//Vector3.Dot(transform.forward, hit.normal) < 0.75f)
			{
	...
	bool VerifyClimbAroundOuterWallAngle(Vector3 normal)
	{
		return Vector3.Dot(transform.forward, normal) <= m_maxClimbAroundOuterAngle; //if dot == -1: direction will have missed the wall since it would be parallell to the direction
	}

	bool VerifyClimbAroundInnerWallAngle(Vector3 normal)
	{
		return Vector3.Dot(transform.forward, normal) <= m_maxClimbAroundInnerAngle; //Not really working as intended ATM because of methods conflicting with eachother
	}
