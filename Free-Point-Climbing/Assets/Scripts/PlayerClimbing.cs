...
public class PlayerClimbing : PlayerStateInterface
{
	...
	bool CheckForWallRays()
	{
		...
		{
			if (hit.transform.CompareTag("Climb"))// && VerifyClimbAroundWallAngle(hit.normal, false))
			{
				return true;
			}
		}
		...
		{
			if (hit.transform.CompareTag("Climb"))// && VerifyClimbAroundWallAngle(hit.normal, true))
			{
				return true;
			}
		}
	...
	bool TryGoAroundWall()
	{
		...
		if (Physics.Raycast(rayDirection, out hit, 1f))
		{
			...
			if (VerifyClimbAroundWallAngle(hit.normal, false))//Vector3.Dot(transform.forward, hit.normal) < 0.75f)
			{
		...
		{
			//If normal compared to player forward isn't too steep
			if (VerifyClimbAroundWallAngle(hit.normal, true))//Vector3.Dot(transform.forward, hit.normal) < 0.75f)
			{
				transform.position = Vector3.Lerp(transform.position, hit.point + hit.normal * m_normalOffset, 0.7f);
				Quaternion look = Quaternion.LookRotation(-hit.normal, transform.up);
				transform.rotation = Quaternion.Slerp(transform.rotation, look, 0.7f);
	...
	bool VerifyClimbAroundWallAngle(Vector3 normal, bool verifyWithAngled)
	{
		if (verifyWithAngled)
		{
